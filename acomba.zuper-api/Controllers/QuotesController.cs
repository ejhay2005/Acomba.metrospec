...
namespace acomba.zuper_api.Controllers
{
    ...
    public class QuotesController : ControllerBase
    {
        ...
        [HttpGet("get-estimate")]
        public async Task<IActionResult> GetEstimate(int count, int page)
        {
            try
            {
                using (var http = new HttpClient())
                {
                    ...
                    HttpRequestMessage request = new HttpRequestMessage(HttpMethod.Get, $"{ZuperUrl}estimate?count={count}&page={page}");
                    HttpResponseMessage response = await http.SendAsync(request);
                    var responseBody = response.Content.ReadAsStringAsync().Result;
                    var result = JsonConvert.DeserializeObject<QuotesResponse>(responseBody);
